...
(function () {
    ...
    var q = [];
    var busy = false;
    var session = Date.now().toString(36) + "-" + Math.floor(Math.random() * 0x100000000).toString(36);
    function sendOne(line) {
        if (busy) { if (q.length < 64) q.push(line); return; }
        busy = true;
        try {
            if (typeof navigator.sendBeacon === "function") {
                if (navigator.sendBeacon(bUrl, "slop1360 " + line)) { busy = false; return; }
            }
        } catch (e) {}
        try {
            var im = new Image();
            im.src = bUrl + "?d=" + encodeURIComponent(line.slice(0, 1500));
        } catch (e2) {}
        busy = false;
    }
    function pump() { while (q.length) { var l = q.shift(); sendOne(l); } }
    setInterval(pump, 500);
    window.__jbSend = function (tag, detail) {
        try {
            var t = String(tag).replace(/[^A-Za-z0-9_.:-]/g, "_").slice(0, 48);
            var d = String(detail).replace(/[^A-Za-z0-9_.:=,+:\/\\\-]/g, "_").slice(0, 320);
            sendOne(session + " " + (window.fw_str || "?") + " " + t + " " + d);
        } catch (e) {}
    };
    window.onerror = function (msg, src, ln, col, err) {
        try {
            window.__jbSend("WINDOW-ERROR", String(msg) + "@" + String(src) + ":" + String(ln)
                + " " + (err && err.stack ? String(err.stack).replace(/[^A-Za-z0-9_.:=,+:\/\\\- ]/g, "_").slice(0, 200) : ""));
        } catch (e) {}
    };
    try {
        if (typeof URLSearchParams === "function") {
            window.__stopAfter = new URLSearchParams(location.search).get("stop");
        }
    } catch (e) {}
})();
/* ============ END INSTRUMENTATION ============ */
...
const supportedFirmwares = [
    ...
    "10.40", "10.60", "11.00", "11.20", "11.40", "11.60", "12.00", "13.60"
];
...
function jbmark(tag, detail) {
    ...
    if (typeof window.__jbSend === "function") {
        try { window.__jbSend("JB-" + tag, String(detail)); } catch (e) {}
    }
}
...
async function prepare(p) {
    ...
    if (window.__stopAfter === "leaks") {
        jbmark("STOP-LEAKS", "voluntary-halt-after-base-leak-wk=0x"
            + libSceNKWebKitBase.toString()
            + "-lk=0x" + libKernelBase.toString());
        throw new Error("voluntary halt after base leak: "
    ...
    if (window.__stopAfter === "worker") {
        jbmark("STOP-WORKER", "voluntary-halt-after-find_worker-stack=0x"
            + worker_stack.toString());
        throw new Error("voluntary halt after find_worker: "
            + worker_stack.toString());
    }
